...
export default function BillEditScreen() {
  ...
  const { pendingBillItems, addBill, setPendingBillItems } = useAppData();
  ...
  const updateItem = useCallback(
    (index: number, field: 'quantity' | 'price', value: string) => {
      ...
      setItems((prev) => {
        const next = [...prev];
        next[index] = { ...next[index], [field]: field === 'quantity' ? Math.floor(num) : num };
        return next;
      });
    },
    []
  ...
  const generateBill = useCallback(() => {
    ...
    const billId = addBill(valid, total);
    router.replace({ pathname: '/bill-preview', params: { id: billId } });
  }, [items, total, addBill, router]);

  if (items.length === 0) {
    return (
      <SafeAreaView style={styles.container} edges={['top']}>
        <ThemedView style={styles.empty}>
          <ThemedText>No items to bill. Go back and capture products.</ThemedText>
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        <View style={styles.header}>
          ...
          <ThemedText style={styles.subtitle}>Adjust quantities and prices</ThemedText>
        </View>
        ...
        <View style={[styles.footer, { borderTopColor: colors.icon + '40' }]}>
          ...
          >
            <ThemedText style={styles.generateBtnText}>Generate Bill</ThemedText>
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
    </SafeAreaView>
